class Fixtures::Soccer::PreMatchController < ApplicationController
   ...
   def index
      if params[:q].present?
         Fixture.where("status = ? AND sport_id = ? AND category_id NOT IN (?)", "not_started", "sr:sport:1", ['sr:category:1033','sr:category:2123']).ransack(params[:q])
      else
         ...
         @q = Fixture.where("status = ? AND sport_id = ? AND category_id NOT IN (?)", "not_started", "sr:sport:1", ["sr:category:1033","sr:category:2123"]).ransack(params[:q])
      end
      ...
      respond_to do |format|
         ...
         format.js
         format.json {
            render json: { fixtures: render_to_string(partial: "pre_match_fixture_table", formats: [:html]), pagination: view_context.pagy_nav(@pagy) }
         }
      end
