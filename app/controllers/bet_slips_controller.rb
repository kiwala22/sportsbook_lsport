			...
			if stake <= current_user.balance
				...
				@cart.line_bets.each do |bet|
					...
					if fetch_market_status(bet.market, bet.fixture_id) == "Active"
						...
						bets_arr << {user_id: current_user.id ,bet_slip_id: bet_slip.id,fixture_id: bet.fixture_id,outcome_id: bet.outcome,market_id: market_id , odds: odd, status: "Pending", product: product, outcome_desc: bet.description, specifier: fetch_specifier(bet.market, bet.fixture_id) }
					end
				end
