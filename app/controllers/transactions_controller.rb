class TransactionsController < ApplicationController
  ...
  def perform_deposit
    ...
    if @transaction.save
      ...
      render json: {message: "Please wait while we process your transaction..."}, status: 200
      # redirect_to root_path
      # flash[:notice] = "Please wait while we process your transaction.."
    else
      render json: {message: "Transaction Has failed please try again."}, status: 400
  ...
  def perform_withdraw
    ...
    if current_user.deposits.any? # && current_user.bets.any?
      ...
      if (@transaction.amount > user_balance)
        render json: {message: "You have insufficient funds on your account."}, status: 400
        # redirect_to root_path
        # flash[:alert] = "You have insufficient funds on your account."
      else
        if @transaction.save
          WithdrawsWorker.perform_async(@transaction.id)
          render json: {message: "Please wait while we process your payment..."}, status: 200
          # redirect_to root_path
      ...
      render json: {message: "You need to make a Deposit or place a bet before any withdraw."}, status: 400
