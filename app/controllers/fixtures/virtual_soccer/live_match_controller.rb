class Fixtures::VirtualSoccer::LiveMatchController < ApplicationController
   ...
   def index
      @q = Fixture.joins(:market1_live).where("fixtures.status = ? AND fixtures.sport_id = ? AND fixtures.location_id IN (?) AND market1_lives.status = ? ", "live", "6046", [], "Active").order(start_date: :asc)
