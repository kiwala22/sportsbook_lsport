class Fixtures::Soccer::PreMatchController < ApplicationController
   ...
   def index
     if params[:q].present?
       @check_params = false
       parameters = ["fixtures.status= 'not_started'", "fixtures.sport_id='6046'", "fixtures.location_id NOT IN ('123')", "market1_pres.status = 'Active'", "fixtures.start_date >= '#{Time.now}'" ]
       ...
       @check_params = false
       @q = Fixture.joins(:market1_pre).where("fixtures.status = ? AND fixtures.sport_id = ? AND fixtures.location_id NOT IN (?) AND fixtures.start_date >= ? AND fixtures.start_date <= ? AND market1_pres.status = ?", "not_started", "6046", "(123)", (Time.now), (Date.today.end_of_day + 1.days), "Active").order(start_date: :asc)
     end
